...
const FAST_DISCOVERY_READ_CONCURRENCY = 64;
const FAST_DISCOVERY_RESOLVE_CONCURRENCY = 64;
const FAST_DISCOVERY_FILE_CONCURRENCY = 256;
...
export async function fastDiscoverEntries({
  ...
  const readSource = async (filePath: string): Promise<string | null> => {
    ...
    const source = await readLimit(async () => {
      try {
