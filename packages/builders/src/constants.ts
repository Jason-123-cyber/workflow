 ...
 *
 * Builds on the (optionally namespaced) `WORKFLOW_QUEUE_TRIGGER`. When
 * `WORKFLOW_SEQUENTIAL_REPLAYS` is enabled, sets `maxConcurrency: 1` so VQS
 * processes at most one flow invocation per concrete topic at a time. Paired
 * with the per-run physical topic naming in `@workflow/world-vercel` (which
 * appends the run id to the flow topic), this enforces at most one orchestrator
 * invocation per run. Step routes are intentionally excluded.
 *
